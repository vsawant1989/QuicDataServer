...
use futures_util::{AsyncRead, AsyncWriteExt, Stream, StreamExt, TryFutureExt};
...
struct Opt {
    ...
    #[structopt(long = "listen", default_value = "[::1]:8000")]
    listen: SocketAddr,
}
...
async fn run(options: Opt) -> Result<()> {
    let (certs, key) = if let (Some(key_path), Some(cert_path)) = (&options.key, &options.cert) {
        ...
        let cert_path =   PathBuf::from("resources/certs/cert.der");
        let key_path =   PathBuf::from("resources/certs/key.der");
        let (cert, key) = match fs::read(&cert_path).and_then(|x| Ok((x, fs::read(&key_path)?))) {
            ...
            Err(ref e) if e.kind() == io::ErrorKind::NotFound => {
                ...
                fs::create_dir_all(Path::new("resources/certs")).context("failed to create certificate directory")?;
...
async fn handle_request(
    ...
    let s = match str::from_utf8(req.as_slice()) {
        Ok(v) => v,
...
fn process_get(root: &Path, x: &[u8]) -> Result<Vec<u8>> {
    ...
    let mut real_path = PathBuf::from(Path::new("."));
